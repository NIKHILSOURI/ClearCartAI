...
# Global image list and index for sequential browsing
_all_images = None
_current_index = -1
...
def _next_image() -> Optional[Path]:
    ...
    if not all_imgs:
        print(f"[DEBUG] No images found in {cfg.raw_dir}")
        return None
    
    _current_index = (_current_index + 1) % len(all_imgs)
    p = all_imgs[_current_index]
    
    labeled = _already_labeled()
    label_status = "✓ labeled" if p.stem in labeled else "unlabeled"
    print(f"[DEBUG] Load Next: [{_current_index + 1}/{len(all_imgs)}] {p.name} ({label_status})")
...
def run_sam(state):
    ...
    try:
        ...
        if state.get("uid") != uid0:
            return None, state, "⚠️ Image changed during SAM run. Result discarded. Please run again."
...
with gr.Blocks(title="SAM3 Product Labeler") as demo:
    ...
    img = gr.Image(label="Image / Mask Overlay", interactive=True)
